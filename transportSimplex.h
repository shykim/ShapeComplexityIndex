...
public:
	...
	std::vector<float> features;							// Pointer to the features vector 
	...
	TsSignature(int nin, std::vector<float> fin, std::vector<float> win):n(nin), features(fin), weights(win){};
};
...
double transportSimplex(TsSignature<TF> *signature1, TsSignature<TF> *signature2, double (*grndDist)(float, float), TsFlow *flowTable = NULL, int *flowSize = NULL) {
